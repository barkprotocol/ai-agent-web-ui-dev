...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!message) return NextResponse.json({ error: "No message found" }, { status: 400 })
    ...
    return createDataStreamResponse({
      execute: async (dataStream) => {
        ...
        const tools = toolsRequired ? getToolsFromRequiredTools(toolsRequired) : defaultTools
            ...
            try {
              ...
              const saved = await dbCreateMessages({
                messages: finalMessages.map((m) => ({
                  ...
                  role: m.role,
                  content: m.content,
                  toolInvocations: m.toolInvocations ? JSON.parse(JSON.stringify(m.toolInvocations)) : undefined,
